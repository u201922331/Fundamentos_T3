		...
		for (int x = 0; x < _levelData[y].size(); x++)
		{
			...
			case 'B':
				_spriteBatch.draw(destRect, uvRect, 
					ResourceManager::getTexture("assets/textures/red_bricks.png").id
					,0.0f,color);
				break;
			case 'G':
				_spriteBatch.draw(destRect, uvRect,
					ResourceManager::getTexture("assets/textures/glass.png").id
					, 0.0f, color);
				break;
			case 'L':_spriteBatch.draw(destRect, uvRect,
				ResourceManager::getTexture("assets/textures/light_bricks.png").id
				, 0.0f, color);				
				break;
			case '@':
				_levelData[y][x] = '.';
				...
				break;
			case 'Z':
				_zombiesPosition.push_back(glm::vec2(x, y));
				break;
			case '.':
				break;
			default:
